...
import { initApiUrl, initAuthToken, initLanguage, setUnauthHandler, logout,
         registerPushToken }             from './src/api/client';
import { registerForPushNotifications, initNotificationHandler } from './src/utils/notifications';
...
function App() {
  ...
  // Avant : `if (!fontsLoaded) return null;` placé AVANT useEffect → violation
  // des Rules of Hooks → "Runtime not ready" sur Expo Go.
  useEffect(() => {
  ...
  // Splash pendant init — pas de blocage sur fonts (chargement en tâche de fond)
  if (appState === null) {
    return (
      <SafeAreaProvider>
        <View style={{ flex:1, alignItems:'center', justifyContent:'center', backgroundColor:'#F7F5F2' }}>
          <Text style={{ color:'#C45A2D', fontSize:32, fontWeight:'700', letterSpacing:4 }}>LEXAVO</Text>
          <Text style={{ color:'#1C2B3A', fontSize:13, marginTop:12 }}>Initialisation...</Text>
        </View>
      </SafeAreaProvider>
    );
  }
